const { copy } = require("../controllers/usersController");
...
const goatRanker = (array, results) => {
  ...
  votes.map((item, index) => {
    for (let i = 0; i < copyArray.length; i++) {
      if (item.id === copyArray[i].artistName) {
        item.artistId = copyArray[i].artistId;
      }
    }
  });
...
// console.log(goatRanker(data.userVotes, 10));
