...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/goatrankerfinal4";
...
*/ app.get(
  ...
  async (req, res) => {
    try {
      ...
      const foundTracks = await spotifyApi.getArtistTopTracks(
        req.params.id,
        "GB"
      );
...
app.post("/categories/:genre", async (req, res) => {
  try {
    ...
    const catCheck = await Category.findOne(
      { name: req.body.name },
      (err, foundCategory) => {
        if (err) {
          console.log(err);
    ...
    const assignToUser = await User.findByIdAndUpdate(
